...
export class PostgresCheckpointer {
    ...
    async put(snapshot) {
        ...
        await pool.query(`INSERT INTO ${this.tableName} (thread_id, node_id, state, timestamp, version) VALUES ($1, $2, $3, $4, $5)`, [
            snapshot.threadId,
            snapshot.nodeId,
    ...
    async get(threadId) {
        const pool = await this.getPool();
        const res = await pool.query(`SELECT node_id, state, timestamp, version FROM ${this.tableName} 
    ...
    async delete(threadId, nodeId) {
        ...
        if (nodeId === undefined) {
            ...
            await pool.query(`DELETE FROM ${this.tableName} WHERE thread_id = $1 AND node_id = $2`, [threadId, nodeId]);
        }
    }
}
//# sourceMappingURL=checkpointer-postgres.js.map