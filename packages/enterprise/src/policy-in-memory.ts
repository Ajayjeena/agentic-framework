...
export class InMemoryPolicyEngine implements PolicyEngine {
  private rules: InMemoryPolicyRule[] = [];
  private defaultAllow = false;

  constructor(options?: { rules?: InMemoryPolicyRule[]; defaultAllow?: boolean }) {
    if (options?.rules) this.rules = options.rules;
    if (options?.defaultAllow !== undefined) this.defaultAllow = options.defaultAllow;
  }
